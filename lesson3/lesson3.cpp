...
private:
    ...
    void decryp1() override
    {
        ...
        for (int i = 0; i < encr.at("text").size(); i++)
        {
			...
			if (checkformistake(encryptword, alph1) == 1)
			{
    ...
    void key_generation() override
    {
        ...
        for (int i = 0; i < alph2.size(); i++)
        {
			...
			if (chipletter <= 192 || chipletter >= 255)
			{
    ...
    int checkformistake(string alph, string word) 
    {
        for (int i = 0; i < word.size(); i++)
        {
            int trigger = 0;
			if (word[i] <= 192 || word[i] >= 255)
			{
				trigger = 2;
			}
            ...
            if (trigger == 0)
            {
                cout << ">> Ошибка:: используемый в тексте символ '" << word[i] << "' отсутствует в алфавите";
                return 0;
            }
			if (trigger == 2)
			{
...
private:
	...
	int checkformistake(string word)
	{
		for (int i = 0; i < word.size(); i++)
		{
			int trigger = 0;
...
private:
    ...
    void decryp1() override
    {
        ...
        {
            encryptword = encr.at("text").at(l);
			if (checkformistake(encryptword, alph2) == 0)
			{
				return;
			}
